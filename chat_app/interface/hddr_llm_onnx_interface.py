...
from app_modules.utils import convert_to_markdown, is_stop_word_or_prefix, shared_state, system_prompt
...
# {input}<|eot_id|><|start_header_id|>assistant<|end_header_id|>"""
        ...
        # else:
        self.enable_history_max = 4
        self.template_header = f'<|system|>{system_prompt}<|end|>'
        self.history_template = "<|user|>{input}<|end|><|assistant|>{response}<|end|>"
        self.chat_template = "<|user|>{input}<|end|><|assistant|>"
